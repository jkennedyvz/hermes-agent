...
| `ha_get_history` | Get state history for a Home Assistant entity. Returns a compact summary (change count, first/last state, distinct values, numeric min/max/avg). Set `include_details=true` for full state list. | — |
